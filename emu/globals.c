...
/* ★ Opened READ-ONLY, and cpu.c gates write-back on the device not being the
   ...
   returns, and the machine stops before they drain -- exactly two blocks land
   (superblock, one inode) and no file data, leaving files present, correctly
   sized and full of NULs.  Change an image the way it is built: attach it as
   sd0b from another boot and `umount', which waits. */
FILE *root_img;
